...
def save_flavors() -> None:
    ...
    for flavor in flavors:
        ...
        # we make a search with contains and check all aggregates name in lower case 
    ...
    for server in servers:
        ...
        for key in server['addresses']:
            for address in server['addresses'][key]:
                ...
                try:
                    address_obj = ServerAddresses.objects.get(addr=address['addr'])
...
def save_instance_actions() -> None:
    ...
    for server in servers:
        ...
        for action in actions:
            ...
            try:
                action_obj = InstanceActions.objects.get(request_id=action['request_id'])
